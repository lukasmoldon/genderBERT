...
def train_model(epochs, learning_rate, batch_size, max_tokencount=510, truncating_method="head", model_train=True, save_model=True, preload_model=None, val_rows=None, load_embeddings=None, num_rows_train=None,
    ...
    if load_embeddings is not None:
        ...
        if num_rows_train is not None:
            train_df = prepare_data("../datasets/amazon/User_level_train.csv", True, num_rows=num_rows_train, max_tokencount=max_tokencount, truncating_method=truncating_method)
        else:
            train_df = prepare_data("../datasets/amazon/User_level_train.csv", True, max_tokencount=max_tokencount)
        if val_rows is not None:
            val_df = prepare_data("../datasets/amazon/User_level_validation.csv", True, num_rows=val_rows, max_tokencount=max_tokencount, truncating_method=truncating_method)
        else:
            val = prepare_data("../datasets/amazon/User_level_validation.csv", True, max_tokencount=max_tokencount)
